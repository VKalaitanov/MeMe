...
async def exists_user_by_username(username: str, session: AsyncSession):
    ...
    user = await session.scalar(select(Users).where(Users.username == username))  # type: ignore
    if user:
...
async def exists_user_by_phone(number_phone: str, session: AsyncSession):
    ...
    user = await session.scalar(select(Users).where(Users.phone == phone))  # type: ignore
    if user is not None:
